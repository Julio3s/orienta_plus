"""Service d'integration avec l'API Groq."""
...
REFUS_HORS_ORIENTATION = (
    "Je ne reponds qu'aux questions liees a l'orientation universitaire et post-bac au Benin "
...
SYSTEM_PROMPT = """Tu es O+, l'assistant ORIENTA+ : tu ne reponds QU'aux questions liees a l'orientation universitaire et post-bac au Benin.
...
class GroqOrientationService:
    """Service pour interagir avec Groq pour le chatbot O+."""
    ...
    def _get_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "base_url": base_url,
            "model": model,
            "max_tokens": int(getattr(settings, "GROQ_MAX_TOKENS", 800)),
            "temperature": float(getattr(settings, "GROQ_TEMPERATURE", 0.7)),
            "timeout": float(getattr(settings, "GROQ_REQUEST_TIMEOUT", 30.0)),
            "prefer_responses_api": bool(getattr(settings, "GROQ_USE_RESPONSES_API", False)),
            "store_conversations": bool(getattr(settings, "GROQ_STORE_CONVERSATIONS", False)),
        }
    ...
    def _get_client(self) -> tuple[OpenAI | None, dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                "Client Groq initialise sur %s avec le modele %s.",
                config["base_url"],
                config["model"],
            )
        except Exception:
    ...
    def get_response(
        ...
        try:
            ...
            logger.warning("Rate limit Groq atteint.")
            return {
                "reponse": (
                ...
                "source": "error",
                "mode": "groq_rate_limit",
                "error_code": "rate_limit",
            }
        except AuthenticationError:
            logger.error("Authentification Groq invalide.")
            return {
                "reponse": (
                    "La configuration de l'assistant O+ doit etre corrigee. "
                    "Merci de verifier la cle Groq."
                ),
                "source": "error",
                "mode": "groq_auth_error",
                "error_code": "invalid_api_key",
            }
        except (APIConnectionError, APIError):
            logger.exception("Erreur Groq pendant l'appel du chatbot.")
            return self._fallback_payload(message_clean, contexte, error_code="groq_api_error")
        except Exception:
            logger.exception("Erreur inattendue pendant l'appel Groq.")
...
"""
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "Tu es un expert en orientation universitaire et post-bac au Benin. "
        ...
        try:
            ...
            logger.exception("Erreur pendant l'analyse de profil Groq.")
            return self._fallback_profile_analysis(notes, serie, results)
    ...
    def _generate_response(
        ...
        if config["prefer_responses_api"]:
            try:
                ...
                if response_text:
                    return response_text, "groq_responses"

                logger.warning("La Responses API Groq a retourne une reponse vide, repli sur chat.completions.")
            except Exception:
                logger.warning(
                    "Echec de la Responses API Groq, repli sur chat.completions.",
                    exc_info=True,
                )
        ...
        try:
            ...
            if "model" in message and "not found" in message and config.get("model") != fallback_model:
                logger.warning(
                    "Modele Groq introuvable (%s). Reessai avec '%s'.",
                    config.get("model"),
                    fallback_model,
                )
                completion = client.chat.completions.create(
        ...
        return response_text or self._fallback_response(messages[-1]["content"], None), "groq_chat_completions"
    ...
    def _fallback_response(self, message: str, contexte: dict[str, Any] | None = None) -> str:
        ...
        if any(word in message_lower for word in ["bonjour", "salut", "hello", "coucou", "hey"]):
            return (
                ...
                "Le service Groq est momentanement indisponible, mais je peux deja vous guider "
...
groq_service = GroqOrientationService()
